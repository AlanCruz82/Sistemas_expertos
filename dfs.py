...
def recorrido_profundidad(grafo,nodo_inicial,nodo_final,sentido):
    ...
    while pila and not(nodo_meta):
        print(pila) # Mostramos el estado de la pila antes de eliminar el tope
        nodo_visitado = pila.pop() # Eliminamos el tope de la pila LIFO
        ...
        if nodo_visitado == nodo_final:
            ...
            for hijo in hijos:
                if(hijo[0] not in visitados and hijo[0] not in pila):
                    pila.append(hijo[0])
                    padres[hijo[0]] = nodo_visitado
